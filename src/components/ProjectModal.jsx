      ...
      {project && (
        ...
        >
          ...
          >
            ...
            {project.traction && <section className="px-6 py-10 sm:p-12 border-b border-white/10 bg-gradient-to-r from-[#1b0710] to-[#0d0b20]"><p className="case-label text-[#ff5269]">Real numbers from the business</p><div className="mt-3 flex flex-col lg:flex-row lg:items-end lg:justify-between gap-4"><h3 className="text-white text-3xl sm:text-4xl font-black">{project.traction.title}</h3><p className="text-secondary max-w-xl leading-6">{project.traction.description}</p></div><div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4 mt-8">{project.traction.metrics.map((metric) => <article key={metric.label} className="rounded-2xl border border-white/10 bg-black/20 p-6"><strong className="block text-white text-2xl sm:text-3xl">{metric.value}</strong><span className="block text-[#ff7286] font-semibold mt-2">{metric.label}</span><small className="block text-secondary mt-2">{metric.note}</small></article>)}</div></section>}

            {project.productAreas && <section className="px-6 py-10 sm:p-12 border-b border-white/10"><p className="case-label">Inside the product</p><h3 className="text-white text-3xl sm:text-4xl font-black mt-3">What I built and run</h3><div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4 mt-8">{project.productAreas.map((area, index) => <article key={area.title} className="rounded-2xl border border-white/10 bg-white/[0.035] p-6"><span className="text-[#a87cff] text-xs font-black">0{index + 1}</span><h4 className="text-white text-lg font-bold mt-4">{area.title}</h4><p className="text-secondary text-sm leading-6 mt-3">{area.text}</p></article>)}</div></section>}
            ...
            <div className="px-6 py-10 sm:p-12 lg:p-14">
              <div className="grid lg:grid-cols-3 gap-10 lg:gap-14">
                ...
                <section><p className="case-label">What I did</p><ul className="case-list">{project.contributions.map((item) => <li key={item}>{item}</li>)}</ul></section>
                <section><p className="case-label">Proof it works</p><ul className="case-list">{project.evidence.map((item) => <li key={item}>{item}</li>)}</ul></section>
              </div>
